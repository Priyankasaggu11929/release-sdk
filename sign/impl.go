...
func (d *defaultImpl) TokenFromProviders(ctx context.Context, logger *logrus.Logger) (string, error) {
	...
	config, err := d.ExecuteOutput("gcloud", "config", "list")
	if err != nil {
		return "", fmt.Errorf("Error finding gcloud config: %w", err)
	}

	tok, err := d.ExecuteOutput("gcloud", "auth", "print-identity-token", "--audiences", "sigstore", "--include-email", "--impersonate-service-account", "1094895200871@cloudbuild.gserviceaccount.com",)
	if err != nil {
		return "", fmt.Errorf("fetching oidc token from environment: %w and %s", err, config)
	}

	accesstok, _  :=  d.ExecuteOutput("gcloud", "auth", "print-access-token", "--impersonate-service-account", "1094895200871@cloudbuild.gserviceaccount.com",)
